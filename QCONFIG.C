	...
	if (ConfigLoaded == FALSE)	{
    	...
    	if (fp != NULL)    {
        	...
        	if (strcmp(cfg.version, VERSION) == 0)    {
				...
				fread(&qCfg, sizeof(struct QuincyConfig), 1, fp);
 		       	fclose(fp);
        	}
        	else	{
				char path[64];
...
void SaveConfig(void)
{
    ...
    if (fp != NULL)    {
		...
		fwrite(&qCfg, sizeof(struct QuincyConfig), 1, fp);
        fclose(fp);
    }
}
...
void SetBreakpointColor(WINDOW wnd)
{
    foreground = HighlightForeground(wnd);
    background = HighlightBackground(wnd);
}

void SetBreakpointCursorColor(WINDOW wnd)
{
    foreground = HighlightForeground(wnd);
    background = SelectBackground(wnd);
}
